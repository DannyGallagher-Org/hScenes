...
namespace Editor
{
    public partial class LevelTool : EditorWindow 
    {
        ...
        [MenuItem ("How/Levels/Level Tool")]
                ...
                if (_showDetails[count])
                {
                    var topBoxRect = EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        GUILayout.BeginVertical();
                        {
                            ...
                            GUILayout.Label(gameScene.Value.AssetBundle.Value);
                            GUILayout.EndHorizontal();
                    ...
                    {
                        GUI.color = new Color(0.7f, 0.4f, 0.3f);
                        ...
                        {
                            GUILayout.Space(15);
                            if (GUILayout.Button("Pack Level", GUILayout.Width(228)))
                                BuildAssetBundles.BuildAssetBundle(gameScene.Value);
                            GUILayout.Space(30);
                        }
        ...
        private void DrawOptionalLayers(KeyValuePair<string, AdditiveScene> gameScene)
        {
            GUILayout.Label("- Mission Layers -", _lesserTitleStyle);

            GUILayout.BeginVertical();
            var count = 0;
            foreach (var mission in gameScene.Value.OptionalLayers)
            {
                GUI.color = Color.white;
                GUILayout.BeginHorizontal();
                GUILayout.Label(mission.Key, GUILayout.Width(300));

                CustomEditorObjects.TwoToneFoldOut(ref _showOptionalDetails[count], "Show Details", "Hide");
                
                if (_showOptionalDetails[count])
                {
                    GUILayout.FlexibleSpace();
                    _deleteString = EditorGUILayout.TextField(_deleteString);
                    if (GUILayout.Button("Remove", GUILayout.Width(60)))
                    {
                        if (_deleteString != "Delete")
                        {
                            Debug.LogWarning("Type 'Delete' in the Box if you really want to delete.");
                            return;
                        }

                        _deleteString = string.Empty;
                        gameScene.Value.OptionalLayers.Remove(mission.Key);
                        EditorUtility.SetDirty(gameScene.Value);
                        return;
                    }
                }
                ...
                if (_showOptionalDetails[count])
                {
                    var split = mission.Value.Split(';');
                
                    foreach (var layer in split)
                    {
                        GUILayout.BeginHorizontal();
                        GUI.color = Color.white;
                        ...
                        {
                            LoadScenePart(gameScene.Value, layer);
                        }
